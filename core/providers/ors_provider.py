...
MAX_LENGTH_CORRECTION_ROUNDS = 7
MAX_LENGTH_CORRECTION_ROUNDS_FALLBACK = 3
LENGTH_CORRECTION_TOLERANCE = 0.015  # stop once within 1.5% of target distance
...
class ORSProvider(RouteProvider):
    ...
    def get_route(
        ...
        if request.is_loop and not request.via_points:
            ...
            # Spur removal is only appropriate for an unconstrained route
            # generated by ORS.  A route containing explicit waypoints is
            # user-authored geometry and may intentionally retrace a road.
        ...
        elif request.via_points and not request.is_loop:
