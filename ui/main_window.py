...
class MainWindow(QMainWindow):
    ...
    def _on_fuse_start_finish(self):
        ...
        # Pin start_location to exact coordinates of start point
        if self.best_route and self.best_route.points:
            start_pt = self.best_route.points[0]
            self.current_request.start_location = f"{start_pt.lat:.6f},{start_pt.lon:.6f}"
            self.map_view.set_start_point(start_pt.lat, start_pt.lon, "S/F")
            self._ensure_via_points_seeded()

        # Let the routing engine (ORS/OSRM) compute the real road-network path for the closing loop segment
